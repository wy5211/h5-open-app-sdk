...
- 打不开时：支持兜底下载 / 第三方分发服务（如 XInstall）
...
XMInstallSDK.init({
  ...
  scheme: 'yourapp://', // App Scheme
  downloadUrl: 'https://your-app-download-page.com', // 下载页面
  extInfo: { path: '/detail', id: 1 }, // 扩展信息
});
...
- `options.id`: 业务应用 ID
- `options.wxAppId`: 微信 AppId（可选）
- `options.universalLink`: Universal Link（可选）
- `options.scheme`: App Scheme（可选）
- `options.downloadUrl`: 下载页面 URL（可选）
- `options.extInfo`: 扩展信息（可选）
- `options.xInstallConfig`: XInstall 配置（可选）
